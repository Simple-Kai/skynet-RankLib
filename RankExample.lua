--[[
  ...
  RankObj:QueryData(uid)  --通过uid查询当前在排行榜的数据，两个返回值，第一个是当前排名，第二个是数据table
...
local function testRankCoreInterface()
  ...
  -- 测试QueryPos()和QueryData()
  local pos, data = rankObj:QueryData(1)
  assert(pos == 4 and data[1] == 100 and data[2] == 200, "QueryPos或QueryData接口返回值错误！")
  ...
  -- 再次测试QueryPos()和QueryData()
  pos, data = rankObj:QueryData(1)
  assert(pos == 1 and data[1] == 400 and data[2] == 500, "QueryPos或QueryData接口返回值错误！")

  rankObj:ForeachByRange(1, 100, function(uid, data1, data2) print(uid, data1, data2) end)
end
